...
app.abrirImpressao = orcId => {
    ...
    <div class="print-page" style="page-break-inside:avoid;">
        <!-- Cabeçalho normal pág 2 -->
        <div class="p-header" style="margin-bottom:8px;"><div>${logoH}</div><div class="p-hinfo">${hInfo}</div></div>
        <hr class="p-div" style="margin:6px 0;">
        <!-- Dados do paciente compactos -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 16px;margin:6px 0 8px;">
            <div style="display:flex;align-items:baseline;gap:5px;">
                <p class="p-label" style="margin-bottom:0;white-space:nowrap;">Paciente</p>
                <p style="font-size:12px;font-weight:700;color:#013425;margin:0;">${orc.paciente}</p>
            </div>
            <div style="display:flex;align-items:baseline;gap:5px;justify-content:flex-end;">
                <p class="p-label" style="margin-bottom:0;white-space:nowrap;">Data</p>
                <p style="font-size:12px;font-weight:700;color:#013425;margin:0;">${fmtData}</p>
            </div>
            <div style="display:flex;align-items:baseline;gap:5px;">
                <p class="p-label" style="margin-bottom:0;white-space:nowrap;">Médico(a)</p>
                <p style="font-size:12px;font-weight:600;color:#013425;margin:0;">${orc.medico}</p>
            </div>
            <div style="display:flex;align-items:baseline;gap:5px;justify-content:flex-end;">
                <p class="p-label" style="margin-bottom:0;white-space:nowrap;">CRM</p>
                <p style="font-size:12px;font-weight:600;margin:0;">${orc.crm||'—'}</p>
            </div>
        </div>
        <hr class="p-gdiv" style="margin:6px 0 10px;">
        <p style="font-size:15px;font-weight:700;color:#013425;margin-bottom:8px;">${orc.protocoloNome||'Itens do Tratamento'}</p>
        ${fasesHtml||'<p style="font-size:13px;color:#bbb;">Nenhuma fase.</p>'}
        <div class="p-total" style="margin-top:10px;padding:12px 16px;">
            <div style="display:flex;justify-content:space-between;font-size:12px;color:#a3b8b1;margin-bottom:3px;"><span>Total bruto:</span><span style="color:#fff;">${app.fmt(orc.valorBruto||0)}</span></div>
            ${(orc.valorCortesias||0)>0?`<div style="display:flex;justify-content:space-between;font-size:12px;color:#a3b8b1;margin-bottom:3px;"><span>Cortesias:</span><span style="color:#fdba74;">- ${app.fmt(orc.valorCortesias)}</span></div>`:''}
            ${(orc.valorCortesias||0)>0?`<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px;padding-bottom:6px;border-bottom:1px solid rgba(255,255,255,0.1);"><span style="color:#fff;font-weight:600;">Valor líquido:</span><span style="color:#fff;font-weight:700;">${app.fmt(liquido)}</span></div>`:'<hr style="border:none;border-top:1px solid rgba(255,255,255,0.1);margin:6px 0;">'}
            <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px;margin-top:6px;"><span style="color:#C5A365;font-weight:600;">À Vista${orc.descAvista>0?` (${orc.descAvista}% off)`:''}:</span><span style="color:#fff;font-weight:700;font-size:16px;">${app.fmt(orc.valorAvista)}</span></div>
        ...
        <div style="text-align:center;margin-top:10px;padding-top:10px;border-top:1px solid #eee;">
            <p style="font-size:10px;color:#aaa;">Proposta válida por 7 dias · ${cfg.nome} · Tel: ${cfg.tel} · WhatsApp: ${cfg.wpp}</p>
        </div>
    </div>`;
